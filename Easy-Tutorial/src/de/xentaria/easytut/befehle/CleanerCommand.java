...
public class CleanerCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		...
		if(cs instanceof ConsoleCommandSender) {
			System.out.println("Consolen dürfen nicht gesäubert werden!");
			return true;
		}
		Player p = (Player) cs;
		...
		if(args.length == 0) {
			...
			Bukkit.broadcastMessage(Main.pre + "Der Chat wurde von §c" + p.getName() + "§b gesäubert.");
			...
			try {
				...
				andererSpieler.sendMessage(Main.pre + "Dein eigener Chat wurde von §c" + p.getName() + "§b gesäubert.");
				p.sendMessage(Main.pre + "Du hast den Chat von §c" + andererSpieler.getName() + "§b gesäubert.");
